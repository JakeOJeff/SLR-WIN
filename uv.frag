...
extern float ambient = 0.2; // ambient strength, set externally, based on position
float distanceFog = 0.7; // ambient strength, set externally, based on position
...
vec4 effect(vec4 color, Image tex, vec2 texcoord, vec2 pixcoord) {
	...
	float attenuation = 50.0;//1.0 / (1 * distance);

    // in LOVE objectColor is vec4
    vec3 result = (ambientcolor + diffuse+spec) * vec3(objectColor) * attenuation * distanceFog;
